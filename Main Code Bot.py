...
from decimal import Decimal, ROUND_DOWN  # Import Decimal for proper floor rounding
    ...
    try:
        ...
        def buy_condition():
            if df['close'].iloc[-1] <= 0.99 * df['lower_band'].iloc[-1]:
                ...
                print(f"Target Buy Price: {0.99 * df['lower_band'].iloc[-1]}")
        ...
        # Check buy order status
            ...
            try:
                ...
                if status == 'FILLED':
                    ...
                    # Floor the executed quantity to the allowed step size
                    executed_qty_dec = Decimal(str(executed_qty))
                    step_dec = Decimal(str(lot_size_filter['stepSize']))
                    quantity = executed_qty_dec.quantize(step_dec, rounding=ROUND_DOWN)
                    qty_str = format(quantity, f'.{max_precision}f')
        ...
        # Place limit buy order if conditions are met
            ...
            if testing_mode:
                ...
                limit_price = 0.99 * lower_band  # Target price as per strategy
                limit_price = round(limit_price / tick_size) * tick_size
                limit_price_str = f"{limit_price:.{price_precision}f}"

                # Calculate quantity for buy order
                lot_size_filter = next(f for f in symbol_info['filters'] if f['filterType'] == 'LOT_SIZE')
                step_size = float(lot_size_filter['stepSize'])
                max_precision = len(lot_size_filter['stepSize'].split('.')[1])
                usdt_balance = float(client.get_asset_balance(asset='USDT')['free'])
                max_quantity = usdt_balance / limit_price
                # Use floor calculation for quantity
